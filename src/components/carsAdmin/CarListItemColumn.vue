<template>
  <td :data-th="TABLE_HEADS[num].name" class="">
    <template v-if="name === 'images'">
      <div
        v-if="store[name]"
        class="car-list-wrapper-column__images car-list-wrapper-column-images"
      >
        <div
          v-for="(image, index) in store[name]"
          :key="index"
    ...
    <template v-else-if="name === 'action'">
      <div class="d-flex align-items-center gap-2 fs-6">
        <router-link :to="`/car/${store.product_id}`" class="p-2 text-warning">
          <i class="bx bx-pencil"></i>
        </router-link>
        <a class="p-2 text-danger" @click.prevent="removeCar(store.product_id)">
          <i class="bi bi-trash-fill"></i>
        </a>
      </div>
    </template>
    <template v-else>
      <template v-if="!columnEdit && store[name] !== 'undefined'">
        {{ store[name] }}
      </template>
      <span
        v-if="store[name] && editThisColumnOnList"
        class="car-list-wrapper-column__edits car-list-wrapper-column-edits"
      >
        <form v-if="columnEdit">
          <input
            v-model="edits"
        ...
        >
          ...
          <i v-else class="lni lni-save p-2 text-warning"></i>
        </button>
      </span>
    </template>
  </td>
...
import {
  LIST_COLUMN_EDIT,
  TABLE_HEADS,
} from '@/components/carsAdmin/constants/constants';
...
export default {
  ...
  methods: {
    ...mapActions('carAdmin', ['SET_EDIT_COLUMN_ON_LIST', 'DELET_CAR_BY_API']),
    ...
    setEditThisColumnOnList() {
      if (
        ...
        this.edits !== null &&
        this.edits !== '' &&
        this.edits !== this.store[this.name]
      ) {
